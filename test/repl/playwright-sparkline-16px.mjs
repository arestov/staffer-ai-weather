...
(async () => {
  ...
  for (let i = 0; i < Math.min(sparklineIcons.length, 6); i++) {
    ...
    console.log(`  Icon ${i}: container=${ic.containerWidth}x${ic.containerHeight}, ` +
  ...
  for (const ic of featuredIcons) {
    console.log(`  --weather-icon-size=${ic.cssSize}, canvas=${ic.canvasInfo ? `${ic.canvasInfo.width}x${ic.canvasInfo.height} (css: ${ic.canvasInfo.styleWidth})` : 'none'}`);
  }
  ...
  const speedInfo = await page.evaluate(() => {
    ...
    const featuredIcon = document.querySelector('.location-card--featured .weather-condition-icon:not(.sparkline-icon-track__icon .weather-condition-icon)');
    return {
      sparklineSpeed: sparklineIcon ? getComputedStyle(sparklineIcon).getPropertyValue('--weather-icon-speed').trim() : 'N/A',
      featuredSpeed: featuredIcon ? getComputedStyle(featuredIcon).getPropertyValue('--weather-icon-speed').trim() : 'N/A',
    };
  });
