...
const main = async () => {
  ...
  try {
    ...
    results.mainPage = await page.evaluate(
      ({ getStylesSrc, getPseudoStylesSrc }) => {
        ...
        // 1b. body::before
        const bodyBefore = getPseudoStyles(document.body, '::before', [
          ...bgProps,
          'content',
          'background-image',
        ])
        ...
        const triggerDotStyles = getStyles(triggerDot, [
          'background',
          'background-color',
          'box-shadow',
          'width',
        ...
        const statusPillStyles = getStyles(statusPill, [
          'background',
          'background-color',
          ...textProps,
        ])
        ...
        const additionalCards = document.querySelectorAll(
          '.location-card:not(.location-card--featured)',
        )
    ...
    results.popover = await page.evaluate(
      ({ getStylesSrc, getPseudoStylesSrc }) => {
        const getStyles = new Function('el', 'props', getStylesSrc)
        const getPseudoStyles = new Function('el', 'pseudo', 'props', getPseudoStylesSrc)
        ...
        const arrowBefore = getPseudoStyles(popover, '::before', [
          ...bgProps,
          'width',
          'height',
          'transform',
    ...
    results.searchPanel = await page.evaluate(
      ({ getStylesSrc }) => {
        const getStyles = new Function('el', 'props', getStylesSrc)
        ...
        const currentLocationBtn = document.querySelector(
          '.selected-location-search__result--current',
        )
    ...
    // Wait for results
    await page
      .waitForSelector('.selected-location-search__result', { timeout: 15000 })
      .catch(() => {})
    await page.evaluate(() => new Promise((r) => setTimeout(r, 300)))

    results.searchResults = await page.evaluate(
      ({ getStylesSrc }) => {
        const getStyles = new Function('el', 'props', getStylesSrc)
