...
const main = async () => {
  ...
  try {
    ...
    console.log('\n=== Test 1: Graceful server leave ===')
    {
      ...
      // Wait for connection
      await waitFor(() => getP2P(page1), s => s.role !== 'undecided', 'page1 role')
      await waitFor(() => getP2P(page2), s => s.role !== 'undecided', 'page2 role')
      ...
      await waitFor(() => getP2P(clientPage), s => s.peerCount >= 1, 'client connected')
    ...
    console.log('\n=== Test 3: New peer joins after failover ===')
    {
      ...
      assert.ok(msg3.receivedMessages.some(m => m.type === 'post-failover-test' && m.value === 999))
    ...
    console.log('\n=== Test 4: Three peers — failover with multiple clients ===')
    {
      ...
      await waitFor(() => getP2P(page1), s => s.role !== 'undecided', 'page1 role')
      await waitFor(() => getP2P(page2), s => s.role !== 'undecided', 'page2 role')
      await waitFor(() => getP2P(page3), s => s.role !== 'undecided', 'page3 role')
      ...
      const results = await Promise.all(
        clientPages.map(p =>
          waitFor(() => getP2P(p), s => s.role !== 'undecided', 'role after failover', 30000)
        ),
      )

      const newRoles = results.map(r => r.role).sort()
      assert.deepEqual(newRoles, ['client', 'server'],
